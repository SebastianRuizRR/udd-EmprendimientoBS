...
/* --FLOW/STATE-- */
type FlowStep="lobby"|"f1_video"|"f1_instr"|"f1_activity"|"f1_rank"|"f2_video"|"f2_instr"|"f2_theme"|"f2_activity"|"f2_rank"|"f3_video"|"f3_activity"|"f3_rank"|"f4_video"|"f4_prep"|"f4_pitch"|"f5_video"|"f5_eval"|"f5_rank"|"f6_video"|"f6_close"|"qr";
type FlowState={step:FlowStep;running:boolean;remaining:number;roomCode:string;expectedTeams:number;};
...
type Analytics={roomsCreated:number;challengeUsage:Record<string,number>;teams:{roomCode:string;teamName:string;integrantes:{nombre:string;carrera:string}[];ts:number;}[];reflections:{roomCode:string;teamName:string;text:string;ts:number;}[];};
...
function useSharedFlow(isTeacher:boolean,initial:FlowState){
  const [flow,setFlow]=useState<FlowState>(()=>readJSON(FLOW_KEY,initial));
  useEffect(()=>{const onStorage=(e:StorageEvent)=>{if(e.key===FLOW_KEY&&e.newValue){try{setFlow(JSON.parse(e.newValue))}catch{}}};window.addEventListener("storage",onStorage);return()=>window.removeEventListener("storage",onStorage)},[]);
  const publish=(next:Partial<FlowState>)=>{const newFlow={...flow,...next};setFlow(newFlow);writeJSON(FLOW_KEY,newFlow);try{window.dispatchEvent(new StorageEvent("storage",{key:FLOW_KEY,newValue:JSON.stringify(newFlow)}))}catch{}};
...
/* --ANALYTICS HOOK-- */
function useAnalytics(){const [analytics,setAnalytics]=useState<Analytics>(()=>readJSON<Analytics>(ANALYTICS_KEY,{roomsCreated:0,challengeUsage:{},teams:[],reflections:[]}));
...
export default function App(){
  ...
  const initialFlow:FlowState={step:"lobby",running:false,remaining:6*60,roomCode:"",expectedTeams:0};
  ...
  const readyCount=()=>{const set=new Set<string>(readJSON<string[]>(READY_KEY,[])); return Array.from(set).filter(id=>id.startsWith(`${flow.roomCode}::`)).length;};
  ...
  useEffect(()=>{if(!teamId||mode!=="alumno")return; const map=readJSON<Record<string,number>>(COINS_KEY,{}); map[teamId]=coins; writeJSON(COINS_KEY,map); try{window.dispatchEvent(new StorageEvent("storage",{key:COINS_KEY,newValue:JSON.stringify(map)}))}catch{};},[coins,teamId,mode]);

  const ranking=useMemo(()=>{const map=readJSON<Record<string,number>>(COINS_KEY,{}); const pairs=Object.entries(map).filter(([id])=>id.startsWith(`${flow.roomCode}::`)).map(([id,v])=>({equipo:id.split("::")[1]||"Equipo",total:v||0})); return pairs.sort((a,b)=>b.total-a.total);},[flow.roomCode,flow.step,storageTick]);
  ...
  function mmss(sec:number){const m=String(Math.floor(sec/60)).padStart(2,"0"); const s=String(sec%60).padStart(2,"0"); return `${m}:${s}`;}
  const readyNow=useMemo(()=>readyCount(),[storageTick,flow.roomCode,flow.expectedTeams]);

  /* ===================== SCREENS ===================== */

  {/* --INICIO (Landing)-- */}
  if(mode==="inicio")return(<div style={appStyles}><Background/><GlobalFormCSS/><AutoCenter>
  ...
  if(mode==="admin"){return(<div style={appStyles}><Background/><GlobalFormCSS/><AutoCenter>
    <AdminDashboard analytics={analytics} THEMES={THEMES} setTHEMES={saveTHEMES} flow={flow} onBack={()=>setMode("inicio")} ranking={ranking}
      clearMetrics={()=>update(()=>({roomsCreated:0,challengeUsage:{},teams:[],reflections:[]}))}/>
  </AutoCenter></div>)}
  ...
  if(mode==="prof"){return(<div style={appStyles}><Background/><GlobalFormCSS/><AutoCenter>
      ...
      <Card title="Crear Nueva Sala" subtitle="Define cantidad de equipos" width={820}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div style={{alignSelf:"end"}}>
            <Btn onClick={()=>{const code=generateCode(); clearReadyForRoom(); writeJSON(COINS_KEY,{}); try{window.dispatchEvent(new StorageEvent("storage",{key:COINS_KEY,newValue:JSON.stringify({})}))}catch{}; publish({roomCode:code,expectedTeams:equiposQty,step:"lobby",remaining:6*60,running:false}); update(a=>({...a,roomsCreated:a.roomsCreated+1}))}} bg={theme.rosa} label="Generar Código"/>
          </div>
        </div>
      </Card>
    ):null}
    ...
    {flow.roomCode&&flow.step==="lobby"&&(
      <Card title="Sala creada" subtitle="Comparte el código y espera a los equipos" width={700}>
        <div style={{fontSize:32,fontFamily:"Roboto Mono, ui-monospace, SFMono-Regular, Menlo, monospace",marginBottom:8,color:theme.azul}}>{flow.roomCode}</div>
        <div style={{fontSize:13,opacity:.8,marginBottom:12}}>Equipos listos: <b>{readyNow}</b> / <b>{flow.expectedTeams}</b></div>
        <Btn onClick={()=>{if(readyNow<flow.expectedTeams)return; setStep("f1_video");}} label="Continuar con todos" disabled={readyNow<flow.expectedTeams}/>
      </Card>
    )}

    {/* --F1 VIDEO-- */}
    {flow.step==="f1_video"&&(<><VideoSpace title="Trabajo en equipo"/><Btn onClick={()=>setStep("f1_instr")} label="Continuar con todos" full={false}/></>)}
    {flow.step==="f1_instr"&&(<><Instructions title="Fase 1 — Trabajo en equipo" bullets={["Juego 1: <b>Spot the Difference</b>.","Juego 2: <b>Matriz de luces</b>.","El tiempo es compartido; ustedes deciden cómo distribuirlo."]}/>
      <Btn onClick={()=>{resetTimer(6*60); setStep("f1_activity",6*60)}} label="Abrir juegos y timer" full={false}/></>)}
    {flow.step==="f1_activity"&&(<Card title="Fase 1 — En curso" subtitle="Timer visible para todos" width={720}><BigTimer label="Tiempo F1 (Diferencias/Matriz)" defaultSec={6*60}/>
    ...
    {/* --F2-- */}
    {flow.step==="f2_video"&&(<><VideoSpace title="Empatía con el usuario"/><Btn onClick={()=>setStep("f2_instr")} label="Continuar con todos" full={false}/></>)}
    {flow.step==="f2_instr"&&(<><Instructions title="Fase 2 — Empatía" bullets={["Elige temática y desafío.","Completen el <b>mapa de empatía</b> (8:00)."]}/>
    ...
    {flow.step==="f2_theme"&&(<Card title="Temáticas y desafío" subtitle="Cuando estén listos, inicia el mapa" width={980}>
      ...
      <div style={{display:"flex",justifyContent:"center",marginTop:12}}>
        <Btn onClick={()=>{update(a=>{const key=`${temaSel}#${desafioIndex}`; const usage={...a.challengeUsage}; usage[key]=(usage[key]||0)+1; return {...a,challengeUsage:usage};}); resetTimer(8*60); setStep("f2_activity",8*60);}} label="Abrir mapa y timer" full={false}/>
      </div>
    </Card>)}
    {flow.step==="f2_activity"&&(<Card title="Fase 2 — En curso" subtitle="Mapa de empatía (timer)" width={720}><BigTimer label="Tiempo F2 (Empatía)" defaultSec={8*60}/>
    ...
    {/* --F3-- */}
    {flow.step==="f3_video"&&(<><VideoSpace title="Creatividad"/><Btn onClick={()=>{resetTimer(12*60); setStep("f3_activity",12*60)}} label="Abrir actividad y timer" full={false}/></>)}
    {flow.step==="f3_activity"&&(<Card title="Fase 3 — En curso" subtitle="Creatividad (timer)" width={720}><BigTimer label="Tiempo F3 (Creatividad)" defaultSec={12*60}/>
    ...
    {/* --F4-- */}
    {flow.step==="f4_video"&&(<><VideoSpace title="Comunicación"/><Btn onClick={()=>{resetTimer(8*60); setStep("f4_prep",8*60)}} label="Preparación (abrir timer)" full={false}/></>)}
    {flow.step==="f4_prep"&&(<Card title="Fase 4 — Preparación del Pitch" subtitle="Timer visible" width={720}><BigTimer label="Tiempo F4 (Preparación)" defaultSec={8*60}/>
      <div style={{display:"flex",justifyContent:"center",marginTop:12}}><Btn onClick={()=>setStep("f4_pitch")} label="Ir a zona de Pitch" full={false}/></div></Card>)}
    {flow.step==="f4_pitch"&&(<Card title="Zona de Pitch" subtitle="Presentaciones de 90s (demo)" width={900}>
      <div style={{width:"100%",aspectRatio:"16/9",background:"#F0F4F8",borderRadius:12,border:`2px dashed ${theme.border}`,display:"grid",placeItems:"center",fontWeight:700,color:"#90A4AE",marginBottom:12,pointerEvents:"none"}}>Área de presentación (sin temporizador)</div>
      <Btn onClick={()=>setStep("f5_video")} label="Continuar con todos" full={false}/>
    </Card>)}

    {/* --F5-- */}
    {flow.step==="f5_video"&&(<><VideoSpace title="Evaluación y retroalimentación"/><Btn onClick={()=>setStep("f5_eval")} label="Abrir evaluación" full={false}/></>)}
    {flow.step==="f5_eval"&&(<Card title="Fase 5 — Evaluación" subtitle="Cada equipo califica (demo)" width={900}>
      <div style={{fontSize:13,opacity:.8,marginBottom:12}}>(En tu versión final conectaremos formularios/criterios; aquí solo continuamos)</div>
      <Btn onClick={()=>setStep("f5_rank")} label="Ver ranking" full={false}/>
    </Card>)}
    {flow.step==="f5_rank"&&(<Card title="Ranking — Fase 5" subtitle="Resultados en vivo" width={900}><RankingBars data={ranking} onContinue={()=>setStep("f6_video")}/></Card>)}

    {/* --F6 / CIERRE-- */}
    {flow.step==="f6_video"&&(<><VideoSpace title="Cierre y reflexión"/><Btn onClick={()=>setStep("f6_close")} label="Ir a cierre" full={false}/></>)}
    {flow.step==="f6_close"&&(<Card title="Cierre y Apoyo" subtitle="Reflexión final (demo)" width={900} tight>
      <div style={{textAlign:"left"}}><p style={{marginTop:0}}>🎉 ¡Felicitaciones! Escribe tu reflexión final.</p>
    ...
    {flow.step==="qr"&&(<Card title="¡Evalúa el juego!" subtitle="Escanea el código QR con tu celular" width={700}>
      ...
      <Btn onClick={()=>{publish({roomCode:"",expectedTeams:0,step:"lobby",remaining:6*60,running:false}); clearReadyForRoom(); writeJSON(COINS_KEY,{}); try{window.dispatchEvent(new StorageEvent("storage",{key:COINS_KEY,newValue:JSON.stringify({})}))}catch{}}} bg={theme.rosa} label="Terminar" full={false}/>
    </Card>)}
  ...
  if(mode==="alumno"){return(<div style={appStyles}><Background/><GlobalFormCSS/><AutoCenter>
    ...
    {!flow.roomCode&&(<Card title="Alumno" subtitle="Ingresa el código de sala para continuar" width={520}>
      ...
      <Btn onClick={()=>{if(!roomCode.trim())return; const f=readJSON<FlowState>(FLOW_KEY,initialFlow); if(!f.roomCode){alert("Aún no hay sala activa. Espera al profesor.");return;} if(f.roomCode!==roomCode.trim()){alert("Código incorrecto. Verifica con el profesor.");return;}}} label="Entrar a la sala"/>
      <Btn onClick={()=>setMode("inicio")} bg={theme.amarillo} fg={theme.texto} label="⬅ Back"/>
    </Card>)}

    {/* --CREAR GRUPO-- */}
    {flow.roomCode&&!teamReady&&(<Card title={`Sala ${flow.roomCode}`} subtitle="Crea tu grupo y marca listo" width={980}>
      <div style={{display:"grid",gridTemplateColumns:isTablet?"1fr":"1fr 1fr auto",gap:10,marginBottom:12}}>
        ...
        <Btn onClick={()=>{if(!groupName.trim())return; setIntegrantes(arr=>[...arr,{nombre:miNombre||"Integrante",carrera:miCarrera||"—"}]); setMiNombre(""); setMiCarrera("");}} bg={theme.rosa} label="Agregar integrante" full={false}/>
      </div>
      <div style={{textAlign:"left",maxHeight:220,overflowY:"auto",padding:8,border:`1px dashed ${theme.border}`,borderRadius:12,background:"#fff"}}>
        ...
        {integrantes.map((p,i)=>(<div key={i} style={{display:"grid",gridTemplateColumns:"40px 1fr 1fr",gap:10,padding:"8px 0",alignItems:"center",borderBottom:i<integrantes.length-1?"1px solid #eee":"none"}}>
          <div style={{fontWeight:800,color:theme.azul}}>{i+1}.</div><div style={{fontWeight:700}}>{p.nombre}</div><div style={{opacity:.8}}>{p.carrera}</div>
        </div>))}
      </div>
    ...
    {/* --ESPERA-- */}
    {flow.roomCode&&teamReady&&flow.step==="lobby"&&(<Card title="Esperando al profesor…" subtitle="Tu grupo está listo" width={720}>
      <div style={{width:"100%",aspectRatio:"16/9",background:"#F0F4F8",borderRadius:12,border:`2px dashed ${theme.border}`,display:"grid",placeItems:"center",color:"#90A4AE",fontWeight:700,marginBottom:12,pointerEvents:"none"}}>Pantalla de espera</div>
      <div style={{fontSize:13,opacity:.85}}>Esperando a que el profesor continúe…</div>
    </Card>)}
    ...
    {flow.step==="f1_activity"&&(<Card title="Fase 1 — Actividades" subtitle={`Tiempo: ${mmss(flow.remaining)} · Monedas: ${coins}`} width={1100}>
      <div style={{position:"sticky",top:12,zIndex:5,background:"transparent",paddingBottom:6,marginBottom:10}}>
        <div style={{display:"flex",gap:8,overflowX:"auto"}}>
          {[{key:"spot",label:"🔎 Diferencias"},{key:"matrix",label:"🔲 Matriz"}].map(t=>{const active=f1Tab===(t.key as "spot"|"matrix");
            return(<button key={t.key} onClick={()=>setF1Tab(t.key as "spot"|"matrix")} style={{padding:"8px 12px",borderRadius:12,border:`2px solid ${active?theme.rosa:theme.border}`,background:active?"#FFF3F7":"#fff",fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>{t.label}</button>);
          })}
        </div>
      </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:isTablet?"1fr":"1fr 1fr",gap:16}}>
            ...
            <div ref={spotRef} onClick={clickSpot} style={{position:"relative",width:"100%",aspectRatio:"16/7",background:"#fff",borderRadius:16,overflow:"hidden",cursor:flow.running?"crosshair":"not-allowed",border:`1px solid ${theme.border}`}}>
              ...
              <div style={{position:"absolute",inset:0,display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"1fr 1fr"}}>
                {["A","B","C","D"].map(z=>(<div key={z} style={{outline:"1px dashed rgba(0,0,0,.06)",padding:6,fontSize:12,color:"#78909C"}}>{z}</div>))}
              </div>
              {diffs.filter(d=>d.found).map((d,i)=>(<div key={i} style={{position:"absolute",left:`${d.x*100}%`,top:`${d.y*100}%`,transform:"translate(-50%,-50%)",width:d.r*200,height:d.r*200,borderRadius:"50%",border:`3px solid ${theme.rosa}`,boxShadow:"0 0 0 4px rgba(233,30,99,.2)"}}/>))}
            </div>
            {/* derecha referencia */}
    ...
    {flow.step==="f2_theme"&&(<Card title="Temática y desafío" subtitle="Esperando al profesor para iniciar el mapa" width={980}>
      <ThemeChallengeSection THEMES={THEMES} temaSel={temaSel} setTemaSel={setTemaSel} desafioIndex={desafioIndex} setDesafioIndex={setDesafioIndex} desafioActual={desafioActual} isTablet={isTablet} onContinue={()=>{}}/>
    </Card>)}
    {flow.step==="f2_activity"&&(<Card title={`Etapa 2 — ${THEMES[temaSel].label}: ${desafioActual.titulo}`} subtitle={`Tiempo: ${mmss(flow.remaining)} · Monedas: ${coins}`} width={1100}>
      <EmpathySection isTablet={isTablet} isMobile={isMobile} bubbleSize={bubbleSize} centerBubbleSize={centerBubbleSize} bubblePositions={bubblePositions}
        EMPATIA_FIELDS={EMPATIA_FIELDS} empatia={empatia} setActiveBubble={setActiveBubble} activeBubble={activeBubble} onEmpatiaChange={onEmpatiaChange}/>
    </Card>)}

    {/* --F3 Alumno-- */}
    {flow.step==="f3_video"&&<VideoSpace title="Creatividad"/>}
    {flow.step==="f3_activity"&&(<Card title="Etapa 3 — Creatividad (LEGO)" subtitle={`Tiempo: ${mmss(flow.remaining)} · Monedas: ${coins}`} width={900}>
      <div style={{display:"grid",gridTemplateColumns:isTablet?"1fr":"1fr 1fr",gap:10,alignItems:"center"}}>
        <div><p style={{marginTop:0}}>Sube una foto de tu solución (demo):</p><input type="file" accept="image/*"/></div>
        <div><p style={{marginTop:0}}>Mini-retos (3 monedas c/u):</p>
          <div style={{display:"grid",gap:8}}>{["Prototipo montado","Solución explicada","Foto clara"].map(r=>(<Btn key={r} onClick={()=>setCoins(c=>c+3)} bg={"#C8E6C9"} fg={"#1B5E20"} label={`✔ ${r}`}/>))}</div>
        </div>
      </div>
    </Card>)}
    ...
    {flow.step==="f4_prep"&&(<Card title="Etapa 4 — Comunicación (preparación)" subtitle={`Tiempo: ${mmss(flow.remaining)}`} width={1100} tight>
    ...
    {flow.step==="f4_pitch"&&(<Card title="Zona de Pitch" subtitle="Esperando el turno del equipo" width={900}>
      <div style={{width:"100%",aspectRatio:"16/9",background:"#F0F4F8",borderRadius:12,border:`2px dashed ${theme.border}`,display:"grid",placeItems:"center",fontWeight:700,color:"#90A4AE",marginBottom:12,pointerEvents:"none"}}>Área de presentación (sin temporizador)</div>
    </Card>)}

    {/* --F5 Alumno-- */}
    {flow.step==="f5_video"&&<VideoSpace title="Evaluación y retroalimentación"/>}
    {flow.step==="f5_eval"&&(<Card title="Etapa 5 — Evaluación" subtitle="Completa las evaluaciones (demo)" width={900}>
      <div style={{fontSize:13,opacity:.8}}>(En producción conectaremos formularios de evaluación.)</div>
    </Card>)}

    {/* --F6 / QR Alumno-- */}
    {flow.step==="f6_video"&&<VideoSpace title="Cierre y reflexión"/>}
    {flow.step==="f6_close"&&(<Card title="Cierre y Apoyo" subtitle={`Monedas finales: ${coins}`} width={900} tight>
      <div style={{textAlign:"left"}}>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",marginTop:8}}>
          <Btn label="Enviar reflexión" full={false} onClick={()=>{const ta=document.getElementById("reflexionText") as HTMLTextAreaElement|null; const text=(ta?.value||"").trim(); if(!text){alert("Escribe una reflexión primero");return;} const teamName=teamId.split("::")[1]||"Equipo";
            update(a=>({...a,reflections:[...a.reflections,{roomCode:flow.roomCode,teamName,text,ts:Date.now()}]})); alert("¡Gracias! Reflexión registrada."); if(ta)ta.value="";}}/>
        </div>
      </div>
    </Card>)}
    {flow.step==="qr"&&(<Card title="¡Evalúa el juego!" subtitle="Escanea el código QR con tu celular" width={700}>
...
function GridView({grid,size,onClickCell,readOnly}:{grid:boolean[][];size:number;onClickCell:(r:number,c:number)=>void;readOnly?:boolean;}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:`repeat(${size}, ${cell}px)`,gap:6,justifyContent:"center"}}>
    {grid.map((row,r)=>row.map((on,c)=>(<button key={`${r}-${c}`} onClick={()=>!readOnly&&onClickCell(r,c)} style={{width:cell,height:cell,borderRadius:8,border:`1px solid ${theme.border}`,background:on?theme.rosa:"#FFFFFF",boxShadow:on?"inset 0 0 0 3px rgba(233,30,99,.15)":"none",cursor:readOnly?"default":"pointer"}}/>)))}
  </div>);
}
...
function ThemeChallengeSection({THEMES,temaSel,setTemaSel,desafioIndex,setDesafioIndex,desafioActual,isTablet,onContinue}:any){
  return(<>
    <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:12,flexWrap:"wrap",position:"relative",zIndex:3}}>
      {(Object.keys(THEMES) as (keyof typeof THEMES)[]).map(key=>{const active=temaSel===key;
        return(<Btn key={key} onClick={()=>setTemaSel(key)} bg={active?theme.azul:"#BBDEFB"} fg={active?theme.blanco:theme.texto} label={THEMES[key].label} full={false}/>);
      })}
    </div>
    <div style={{display:"grid",gridTemplateColumns:isTablet?"1fr":"280px 1fr 360px",gap:16,alignItems:"start"}}>
      <div style={panelBox}>
        ...
        <div style={{display:"grid",gap:8}}>
          {THEMES[temaSel].desafios.map((d:any,idx:number)=>{const active=desafioIndex===idx; return(
            <button key={idx} onClick={()=>setDesafioIndex(idx)} style={{textAlign:"left",padding:10,borderRadius:12,border:`2px solid ${active?theme.rosa:theme.border}`,background:active?"#FFF3F7":"#fff",fontWeight:700,cursor:"pointer"}}>{d.titulo}</button>
          )})}
        </div>
      </div>
...
function EmpathySection(props:any){
  const {isTablet,isMobile,bubbleSize,centerBubbleSize,bubblePositions,EMPATIA_FIELDS,empatia,setActiveBubble,activeBubble,onEmpatiaChange}=props;
  return(<div style={{display:"grid",gridTemplateColumns:isTablet?"1fr":"420px 1fr",gap:16,textAlign:"left"}}>
    <div style={{position:"relative",height:isMobile?300:340,...panelBox,overflow:"hidden"}}>
      <div style={{position:"absolute",left:"50%",top:"55%",transform:"translate(-50%,-50%)",width:centerBubbleSize,height:centerBubbleSize,borderRadius:"50%",background:"#FFF3F7",border:`3px solid ${theme.rosa}`,display:"grid",placeItems:"center",fontWeight:900,color:theme.rosa,textAlign:"center",padding:12}}>Persona</div>
      {EMPATIA_FIELDS.map((f:any)=>{const filled=!!empatia[f.key]?.trim(); const active=activeBubble===f.key; return(
        <button key={f.key} onClick={()=>setActiveBubble(f.key)} style={{position:"absolute",...(bubblePositions[f.key]||{}),width:bubbleSize,height:bubbleSize,borderRadius:"50%",border:`3px solid ${active?theme.azul:theme.rosa}`,background:filled?"#E1F5FE":"#FFFDE7",color:theme.texto,fontWeight:800,cursor:"pointer",boxShadow:active?"0 0 0 6px rgba(25,118,210,.15)":"0 6px 14px rgba(0,0,0,.08)"}}>
          {filled?"✔ ":""}{f.label}
        </button>
      )})}
    </div>
    <div style={panelBox}>
      <div style={{fontWeight:900,color:theme.azul,marginBottom:6}}>{EMPATIA_FIELDS.find((x:any)=>x.key===activeBubble)?.label}</div>
      <textarea value={empatia[activeBubble]} onChange={e=>onEmpatiaChange(activeBubble,e.target.value)} placeholder={`Escribe sobre ${EMPATIA_FIELDS.find((x:any)=>x.key===activeBubble)?.label}...`} style={{...baseInput,minHeight:160}}/>
      <div style={{fontSize:12,opacity:.75,marginTop:8}}>Consejos: ejemplos concretos, verbos en acción, datos o citas del usuario.</div>
      <div style={{fontSize:13,opacity:.85,marginTop:12}}>Completadas: {EMPATIA_FIELDS.filter((f:any)=>empatia[f.key].trim()).length}/{EMPATIA_FIELDS.length}</div>
    </div>
  </div>);
}

/* --CONFETTI-- */
const ConfettiBurst:React.FC=()=>{const [items,setItems]=useState<{id:number;left:number;delay:number;emoji:string}[]>([]);
  useEffect(()=>{const EMOJIS=["🎉","🎊","✨","🏆","🎈","💥","⭐"]; const arr=Array.from({length:28}).map((_,i)=>({id:i,left:Math.random()*100,delay:Math.random()*.8,emoji:EMOJIS[Math.floor(Math.random()*EMOJIS.length)]})); setItems(arr); const t=setTimeout(()=>setItems([]),2500); return()=>clearTimeout(t);},[]);
  return(<div style={{position:"absolute",inset:0,overflow:"hidden",pointerEvents:"none"}}>
    {items.map(p=>(<div key={p.id} style={{position:"absolute",top:-20,left:`${p.left}%`,fontSize:22,animation:`fall ${1.8+Math.random()*.9}s ease-in ${p.delay}s forwards`}}>{p.emoji}</div>))}
  </div>);
};

/* --RANKING-- */
function RankingBars({data,onContinue}:{data:{equipo:string;total:number}[];onContinue:()=>void;}){
  const [mounted,setMounted]=useState(false); useEffect(()=>setMounted(true),[]); const max=Math.max(1,...data.map(d=>d.total||0));
  return(<>
    <div style={{...panelBox}}>
      {data.length===0&&<div style={{opacity:.7}}>Aún no hay datos de equipos…</div>}
      {data.map((r,i)=>{const pct=Math.round((r.total/max)*100); const isFirst=i===0; const isLast=i===data.length-1; const secondOrThird=!isFirst&&!isLast;
        const barBg=isFirst?`linear-gradient(90deg, ${theme.amarillo}, #FFF59D, ${theme.amarillo})`:secondOrThird?`linear-gradient(90deg, ${theme.azul}22, ${theme.azul}66)`:"#CFD8DC";
        return(<div key={r.equipo} style={{display:"grid",gridTemplateColumns:"56px 1fr 90px",gap:8,alignItems:"center",marginBottom:12}}>
          <div style={{fontWeight:900,fontSize:18,color:theme.blanco,textShadow:"0 1px 2px rgba(0,0,0,.3)"}}>
            {i+1}{isFirst&&<div style={{fontSize:22,lineHeight:"16px",animation:"crownFloat 1.6s ease-in-out infinite"}}>👑</div>}
            {isLast&&<div style={{fontSize:18,lineHeight:"16px"}}>🔗</div>}
          </div>
          <div style={{position:"relative"}}>
            <div style={{position:"absolute",left:10,top:-18,fontWeight:800,color:isLast?"#607D8B":theme.texto}}>{r.equipo}{i===1&&data.length>=2?" 🥈":i===2&&data.length>=3?" 🥉":""}</div>
            <div style={{height:28,background:"#F1F5F9",borderRadius:14,overflow:"hidden",boxShadow:"inset 0 0 0 1px #e2e8f0"}}>
              <div style={{height:"100%",width:mounted?`${pct}%`:0,transition:"width .9s ease",background:barBg,backgroundSize:isFirst?"200% 100%":undefined,animation:isFirst?"shimmer 2.4s linear infinite":undefined,borderRadius:14,position:"relative",filter:isLast?"grayscale(.3)":"none"}}/>
            </div>
          </div>
          <div style={{textAlign:"right",fontWeight:900,fontSize:18}}>{r.total}</div>
        </div>)})}
    </div>
    <div style={{display:"flex",justifyContent:"center",gap:10,marginTop:12}}><Btn onClick={onContinue} label="Continuar" full={false}/></div>
  </>);
}

/* --ADMIN DASHBOARD-- */
function AdminDashboard({analytics,THEMES,setTHEMES,flow,onBack,ranking,clearMetrics}:{analytics:Analytics;THEMES:ThemeConfig;setTHEMES:(t:ThemeConfig)=>void;flow:FlowState;onBack:()=>void;ranking:{equipo:string;total:number}[];clearMetrics:()=>void;}) {
  const [tab,setTab]=useState<"resumen"|"temas"|"equipos"|"reflexiones"|"uso"|"ranking">("resumen");
  const exportJSON=(name:string,data:any)=>{const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`${name}.json`; a.click(); URL.revokeObjectURL(url);};
  const totals={equipos:analytics.teams.length,estudiantes:analytics.teams.reduce((acc,t)=>acc+(t.integrantes?.length||0),0),reflexiones:analytics.reflections.length,rooms:analytics.roomsCreated};

  return(<Card title="Panel de Administrador" subtitle="Configura el juego y revisa métricas" width={1100}>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
      {[["resumen","📊 Resumen"],["temas","🎯 Temáticas & Desafíos"],["equipos","👥 Equipos"],["reflexiones","📝 Reflexiones"],["uso","📈 Uso de desafíos"],["ranking","🏆 Ranking/Monedas"]].map(([k,label])=>(
    ...
    {tab==="resumen"&&(<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:12}}>
      {[{k:"Salas creadas",v:totals.rooms},{k:"Equipos registrados",v:totals.equipos},{k:"Estudiantes (aprox.)",v:totals.estudiantes},{k:"Reflexiones recibidas",v:totals.reflexiones}].map(card=>(
        <div key={card.k} style={panelBox}><div style={{fontSize:13,color:theme.muted}}>{card.k}</div><div style={{fontSize:32,fontWeight:900}}>{card.v}</div></div>
      ))}
      <div style={{gridColumn:"1 / -1",...panelBox}}>
        <div style={badgeTitle}>Sala actual</div>
        <div style={{fontFamily:"Roboto Mono, monospace",fontWeight:900,fontSize:18}}>Código: {flow.roomCode||"—"} · Paso: {flow.step} · Equipos esperados: {flow.expectedTeams||"—"}</div>
      </div>
    </div>)}

    {tab==="temas"&&<ThemeEditor THEMES={THEMES} setTHEMES={setTHEMES}/>}

    {tab==="equipos"&&(<div style={{...panelBox,textAlign:"left"}}>
      <div style={badgeTitle}>Equipos registrados</div>
      {analytics.teams.length===0?(<div style={{opacity:.7}}>Sin equipos aún.</div>):(
        <div style={{display:"grid",gap:10}}>
          {analytics.teams.slice().reverse().map((t,i)=>(
            <div key={i} style={{border:`1px solid ${theme.border}`,borderRadius:12,padding:10}}>
              ...
              <div style={{marginTop:6,display:"grid",gap:4}}>{t.integrantes?.map((p,j)=>(<div key={j}>• <b>{p.nombre}</b> — {p.carrera}</div>))}</div>
            </div>
          ))}
        </div>
      )}
    ...
    {tab==="reflexiones"&&(<div style={{...panelBox,textAlign:"left"}}>
      <div style={badgeTitle}>Reflexiones finales</div>
      {analytics.reflections.length===0?(<div style={{opacity:.7}}>Aún no hay reflexiones.</div>):(
        <div style={{display:"grid",gap:10}}>
          {analytics.reflections.slice().reverse().map((r,i)=>(
            <div key={i} style={{border:`1px solid ${theme.border}`,borderRadius:12,padding:10}}>
    ...
    {/* --USO (arreglado paréntesis)-- */}
    {tab==="uso"&&(<div style={{...panelBox,textAlign:"left"}}>
      ...
      {Object.keys(analytics.challengeUsage).length===0?(<div style={{opacity:.7}}>Aún no hay datos de uso.</div>):(
        <div style={{display:"grid",gap:8}}>
          {Object.entries(analytics.challengeUsage).sort((a,b)=>b[1]-a[1]).map(([key,count])=>{
            ...
            return(<div key={key} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8}}>
              <div>{label}</div><div style={{fontWeight:900}}>{count}</div>
            </div>);
          })}
        </div>
      )}
    ...
    {tab==="ranking"&&(<div style={{...panelBox}}>
      <div style={{marginBottom:8,textAlign:"left",...badgeTitle}}>Monedas (sala actual {flow.roomCode||"—"})</div>
...
/* --Theme Editor-- */
function ThemeEditor({THEMES,setTHEMES}:{THEMES:ThemeConfig;setTHEMES:(t:ThemeConfig)=>void;}){
  const [current,setCurrent]=useState<keyof ThemeConfig>("salud"); const [local,setLocal]=useState<ThemeConfig>(THEMES);
  ...
  const updateChallenge=(i:number,patch:Partial<ThemeChallenge>)=>{const next={...local}; next[current].desafios[i]={...next[current].desafios[i],...patch}; setLocal(next);};
  const updatePersona=(patch:Partial<ThemePersona>)=>{const next={...local}; next[current].persona={...next[current].persona,...patch}; setLocal(next);};

  return(<div style={{display:"grid",gap:12}}>
    <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
      {(Object.keys(local) as (keyof ThemeConfig)[]).map(k=>(
        <button key={k} onClick={()=>setCurrent(k)} style={{padding:"8px 12px",borderRadius:12,border:`2px solid ${current===k?theme.azul:theme.border}`,background:current===k?"#E3F2FD":"#fff",fontWeight:800,cursor:"pointer"}}>{local[k].label}</button>
      ))}
    </div>
    ...
    <div style={{display:"flex",justifyContent:"flex-end",gap:8}}><Btn label="Guardar cambios" onClick={save} full={false}/></div>
  </div>);
}

/* --UTILS-- */
function useMediaQuery(q:string){const [m,setM]=useState(false); useEffect(()=>{const mq=window.matchMedia(q); const h=()=>setM(mq.matches); h(); mq.addEventListener("change",h); return()=>mq.removeEventListener("change",h)},[q]); return m;}
function generateCode(){const letters="ABCDEFGHJKLMNPQRSTUVWXYZ"; const nums="123456789"; const pick=(s:string,n:number)=>Array.from({length:n},()=>s[Math.floor(Math.random()*s.length)]).join(""); return pick(letters,2)+pick(nums,3);}
