...
export const Card: React.FC<{
  ...
  <div style={{
    ...
    <h2 style={{ margin: 0, marginBottom: 8, fontSize: 26, fontWeight: 900, color: tema.rosa }}>{title}</h2>
    {subtitle && <p style={{ marginTop: 0, marginBottom: 16, color: tema.azul }}>{subtitle}</p>}
    {children}
  </div>
);
...
}> = ({ onClick, bg = tema.azul, fg = tema.blanco, label, full = true, disabled, variant = "solid" }) => (
  <button
    onClick={() => onClick?.()}
    disabled={disabled}
    style={{
      ...
      padding: "12px 16px",
      borderRadius: 14,
      border: variant === "outline" ? `2px solid ${tema.azul}` : "none",
      cursor: disabled ? "not-allowed" : "pointer",
      fontWeight: 800,
      letterSpacing: 0.2,
      background: disabled ? "#cfd8dc" : (variant === "outline" ? "transparent" : bg),
      color: disabled ? "#607d8b" : (variant === "outline" ? tema.azul : fg),
